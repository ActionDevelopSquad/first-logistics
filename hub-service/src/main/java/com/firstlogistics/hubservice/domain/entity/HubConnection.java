...
public class HubConnection {
    ...
    public void activate() {
        if(this.status == HubConnectionStatus.ACTIVE)
            throw new HubException(HubErrorCode.ALREADY_HUB_CONNECTION_ACTIVE);
        this.status = HubConnectionStatus.ACTIVE;
    }

    public void deactivate() {
        if(this.status == HubConnectionStatus.INACTIVE)
            throw new HubException(HubErrorCode.ALREADY_HUB_CONNECTION_INACTIVE);
    ...
    private static void validateTime(Time time){
        if(time == null)
            throw new HubException(HubErrorCode.INVALID_MINUTES);
    }
    private static void validateDistance(Distance distance){
        if(distance == null)
            throw new HubException(HubErrorCode.INVALID_METERS);
    }
    private static void validateStatus(HubConnectionStatus status){
        if(status == null)
            throw new HubException(HubErrorCode.INVALID_HUB_CONNECTION_STATUS);
    }
